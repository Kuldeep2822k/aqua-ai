...
self.addEventListener('fetch', (event) => {
  ...
  for (const [name, strategyConfig] of Object.entries(CACHE_STRATEGIES)) {
    if (strategyConfig.pattern.test(url.pathname) || strategyConfig.pattern.test(url.href)) {
  ...
  try {
    ...
    if (request.url.endsWith('.js') || request.url.endsWith('.css')) {
      return new Response('/* Resource unavailable offline */', {
        status: 200,
        headers: { 'Content-Type': request.url.endsWith('.js') ? 'text/javascript' : 'text/css' }
      });
    }
...
async function staleWhileRevalidate(request, config) {
  ...
  console.log(`[SW] Stale while revalidate (no cache, waiting): ${request.url}`);
  return fetchPromise;
}
