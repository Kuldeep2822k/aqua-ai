...
const api = axios.create({
  ...
  // 90s timeout: Render free-tier has ~30-50s cold start on first request
  timeout: 90000,
});
...
export const alertsApi = {
  getAll: async (params?: {
    ...
    end_date?: string;
    parameter?: string;
  }): Promise<{ success: boolean; data: Alert[]; pagination: Pagination }> => {
  ...
  getActive: async (params?: {
    severity?: string;
    limit?: number;
  }): Promise<{ success: boolean; data: ActiveAlert[]; count: number }> => {
  ...
  getStats: async (params?: {
    start_date?: string;
    end_date?: string;
  }): Promise<{ success: boolean; data: AlertStats }> => {
