  ...
  const getFilteredData = () => {
    ...
    return sampleData.filter(location => {
      return categoryParams.some(param => {
        ...
        return locationValue.includes(param.toLowerCase()) || 
               locationName.includes(param.toLowerCase()) ||
               (param === 'Heavy Metals' && (locationValue.includes('lead') || locationValue.includes('mercury') || locationValue.includes('iron'))) ||
               (param === 'Industrial' && (locationValue.includes('industrial') || locationValue.includes('pollutant'))) ||
  ...
  return (
    <Box sx={{ position: 'relative', height: height }}>
      ...
      >
        ...
        <Typography variant="caption" sx={{ display: 'block', mt: 1, fontSize: '9px' }}>
          Showing: {parameterCategories[selectedCategory as keyof typeof parameterCategories].join(', ')}
        </Typography>
      </Box>
