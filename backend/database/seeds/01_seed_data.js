...
exports.seed = async function (knex) {
    ...
    if (client !== 'sqlite3') {
        ...
        const parameterCodes = ['pH', 'BOD', 'DO', 'TDS', 'Coliform'];
        const dbParameters = await knex('water_quality_parameters')
            .select('id', 'parameter_code')
            .whereIn('parameter_code', parameterCodes);
        const parameterIdByCode = new Map(dbParameters.map((p) => [p.parameter_code, p.id]));
        ...
        for (let i = 0; i < readings.length; i += batchSize) {
            await knex('water_quality_readings').insert(readings.slice(i, i + batchSize));
        }
        ...
        const alerts = [
            {
                location_id: locIdByName.get('Yamuna River - Delhi'),
                parameter_id: parameterIdByCode.get('BOD'),
            ...
            {
                location_id: locIdByName.get('Mithi River - Mumbai'),
                parameter_id: parameterIdByCode.get('Coliform'),
                alert_type: 'threshold_exceeded',
                severity: 'high',
            ...
            {
                location_id: locIdByName.get('Cooum River - Chennai'),
                parameter_id: parameterIdByCode.get('DO'),
                alert_type: 'threshold_exceeded',
                severity: 'medium',
        ...
        ].filter((a) => a.location_id && a.parameter_id);
    ...
    const alerts = [
        { location_name: 'Yamuna River - Delhi', state: 'Delhi', parameter: 'BOD', value: 28.5, threshold: 10, severity: 'critical', message: 'BOD levels extremely high', is_resolved: false },
        { location_name: 'Mithi River - Mumbai', state: 'Maharashtra', parameter: 'Coliform', value: 4500, threshold: 500, severity: 'high', message: 'High coliform count detected', is_resolved: false },
        { location_name: 'Cooum River - Chennai', state: 'Tamil Nadu', parameter: 'DO', value: 2.1, threshold: 4, severity: 'warning', message: 'Low dissolved oxygen levels', is_resolved: false },
    ];
