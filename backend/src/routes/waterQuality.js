...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    // Optimized: Use Knex for server-side aggregation instead of fetching all rows
    ...
    if (state) {
      baseQuery.where('locations.state', 'ilike', `%${state}%`);
    }
    ...
    const [
      ...
      baseQuery.clone().avg('quality_score as avg_score').first(),
      ...
      // 4. States Monitored
      baseQuery.clone().distinct('locations.state').pluck('locations.state'),

      // 5. Latest Reading and Total Count
      baseQuery
        .clone()
    ...
    res.json({
      ...
      data: {
        ...
        latest_reading: datesAndCount ? datesAndCount.latest_reading : null,
      },
    });
  })
);
